...
# `BoardTopology()` parses the board JSON and rebuilds the adjacency graph;
# it's static for the whole process, so build it once and share it instead
# of re-parsing on every OccupyAction/Fortify description.
_shared_topology: Optional[BoardTopology] = None


def _topology() -> BoardTopology:
    global _shared_topology
    if _shared_topology is None:
        _shared_topology = BoardTopology()
...
class ActionMarker:
    ...
    def set_action(self, action: Action, pre_pending, now_ms: int) -> None:
        terrs = self.action_territories(action, pre_pending)
    ...
    def action_territories(action: Action, pre_pending) -> list[str]:
        ...
        `pre_pending` is the `pending_attack` captured *before* the action ran
        (an OccupyAction needs it to know which two territories were
        involved).
        """
    ...
    def describe_action(action: Action, player_name: str, pre_pending=None) -> str:
        ...
        if isinstance(action, OccupyAction):
            ...
            if pre_pending is not None:
                dest = " into " + _topology().territory_at(pre_pending.to_index)
    ...
    @classmethod
    def action_report(
        cls, action: Action, player_name: str, info=None, pre_pending=None
    ) -> tuple[str, ...]:
        """Multi-line summary of an executed action for the side panel.

        For attacks this includes the dice that were rolled and the
        casualties on each side. Everything else falls back to the one-line
        `describe_action`.
        """
        info = info or {}
        if isinstance(action, ReinforcementAction):
        ...
        if isinstance(action, AttackAction):
            ...
            lines = [
                ...
                f"atk dice: {cls._fmt_dice(info.get('attacker_rolls'))}",
                f"def dice: {cls._fmt_dice(info.get('defender_rolls'))}",
                f"attacker lost {a_loss}, defender lost {d_loss}",
            ]
            if info.get("conquered"):
                lines.append("territory conquered!")
            return tuple(lines)
        return (cls.describe_action(action, player_name, pre_pending),)


__all__ = ["ActionMarker"]
