...
| `test_learned_agent_play.py` | Interactive saved-policy play (`Docs/PlayLearnedAgents.md`) — UI-only learned-seat cycling, player-count-reduction discard, preset-registry validation, policy-only `.pt`/`epNNNNNN` round-trip through `choose_agent.py`'s reused private helpers, real-seat attachment (`epsilon=0.0`, `train_mode=False`, deterministic inference), pre-start validation (accept/missing/kind-mismatch), independent instances for a shared checkpoint, and a mixed human/heuristic/learned `AppLoop` smoke run; `InitScreenState`-level cycling assertion also mirrored in `test_ui.py` |
