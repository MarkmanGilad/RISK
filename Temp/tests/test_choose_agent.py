...
def test_json_write_retries_a_transient_windows_file_lock(tmp_path, monkeypatch) -> None:
    output = tmp_path / "result.json"
    ...
    sleeps = []
    monkeypatch.setattr(Path, "replace", replace_with_one_lock)
    monkeypatch.setattr("risk.learning.choose_agent.time.sleep", sleeps.append)

    _write_json(output, {"complete": True})
